...
public interface ProcedureRepository extends JpaRepository<ProcedureModel, Long> {
    ...
    @Query("SELECT p FROM Procedure p ORDER BY p.name ASC")
    List<ProcedureModel> findAllOrdered();

    @Query("SELECT p FROM Procedure p WHERE p.active = true ORDER BY p.name ASC")
    List<ProcedureModel> findAllActive();

    @Query("SELECT p FROM Procedure p WHERE p.active = false ORDER BY p.name ASC")
