...
public interface PreventionRepository extends JpaRepository<PreventionModel, Long> {
    ...
    @Query("SELECT p FROM PreventionModel p WHERE p.isActive = true ORDER BY p.name ASC")
    List<PreventionModel> findAllActive();

    @Query("SELECT p FROM PreventionModel p WHERE p.isActive = false ORDER BY p.name ASC")
